...
Message Player::sendMessage(const std::string& recipient, const std::string& content,
...
void Player::printPigeons() const {
    ...
    for (int i = 0; i < (int)pigeons.size(); ++i) {
        std::cout << "\n  [" << (i+1) << "] " << pigeons[i].name
                  << " (" << pigeons[i].rarityColor() << pigeons[i].rarityName() << "\033[0m"
                  << ")  OVR: \033[1m" << pigeons[i].stats.overall() << "\033[0m"
                  << "  " << (pigeons[i].isAvailable
...
void Player::printInbox() const {
    ...
    for (const auto& m : inbox) {
        std::string preview = m.content.substr(0, 25) + (m.content.size() > 25 ? "..." : "");
        std::cout << "  " << std::setw(4)  << m.id
                  << std::setw(14) << m.sender
                  << std::setw(14) << m.pigeonName
                  << std::setw(20) << m.statusString()
                  << preview << "\n";
    }
}
...
void Player::printOutbox() const {
    ...
    for (const auto& m : outbox) {
        std::string preview = m.content.substr(0, 20) + (m.content.size() > 20 ? "..." : "");
        std::cout << "  " << std::setw(4)  << m.id
                  << std::setw(14) << m.recipient
                  << std::setw(14) << m.pigeonName
                  << std::setw(20) << m.statusString()
                  << std::setw(16) << (m.status == MessageStatus::InTransit ? m.timeRemainingString() : "-")
                  << preview << "\n";
    }
}
